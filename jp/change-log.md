...
><b>最新の更新は 2022 年 3 月 14 日に行われました。
...
## 機能改善
- 画像の丸みの半径はパーセンテージで指定します (#5312)
- デフォルトを使用する代わりにビューのズーム率を保持します (#14214)

## メンテナンスの更新とバグ修正
**これには以下が含まれますが、これらに限定されません:**
- [Sketch のアセットを取得する] をクリックするとエラーが発生する問題 (#14681)
- Select のポップアップがプレビュー モードで上部に表示されない問題 (#14281)
- アプリを複製するときにカスタム ビューポートが失われる問題 (#13739)
- Card のアバターを削除することができるため、スイッチが正しく動作しない問題 (#9565)
- データ パネルのブレッドクラムが視覚仕様と一致しない問題 (#14652)
- Fluent テーマおよび丸めを設定すると、アイコンが入力をオーバーフローする問題 (#14846)
- ドラッグ時にタブをデタッチするとアプリが正しく動作しない問題 (#13352)
- ボタンはテキストの編集後にサイズ変更される問題 (#14719)
- キャンバスでコンポーネントを選択解除してもアウトライン セクションが縮小されない問題 (#12895)
- ComboBox 項目はプレビューの影響を受け、クリアボタンがない問題 (#5856)
- Select は、開く/閉じるアクションを追加するときに「コンポーネントの選択」リストに表示される問題 (#6403)
- Checkbox および Switch は、none に設定されていてもラベル位置を生成する問題 (#13775)
- ダーク Fluent テーマの丸みを 1 に設定するとライトになる問題 (#15361)
- セルの編集が正しく機能しない問題 (#14342)
- Sketch プラグインからサインアウトできない問題 (#14486)
- アプリ バーのボタンの無効な状態が正しく機能しない問題 (#14315)
